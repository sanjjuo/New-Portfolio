    ...
    return (
        <>
            <div className='p-5 lg:py-10'>
                <h1 className='uppercase text-appColor-secondary text-[70px] text-center font-bold'>
                    My <span className='font-normal app-text'>skills</span></h1>
                <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-5 max-w-[1200px] mx-auto mt-10'>
                    {skills.map((skill) => (
                        <div key={skill.id} className='w-full p-5 flex flex-col items-center justify-center
                        transition transform duration-700 ease-in-out hover:bg-appColor-primary rounded-full cursor-pointer space-y-2 group'>
                            ...
                            <h5 className='font-semibold tracking-wide capitalize'>{skill.title}</h5>
                        </div>
                    ))}
                </div>
            </div>
